    ...
    def get(self):
        ...
        if now_hour in self.random_time:
            ...
            if len(self.name_list) < 2:
                ...
                if self.ticket is None or t_delta > timedelta(minutes=LIMIT_MIN) or (self.ticket_flag is False and t_delta > timedelta(seconds=VALIDATE_SEC)):
                    if self.ticket_flag == False and t_delta > timedelta(seconds=VALIDATE_SEC):
                        name_list_temp = set(self.name_list)
                        name_list_temp.discard(self.ticket)
                        self.name_list = list(name_list_temp)
                    self.ticket = random.choice(self.name_list)
                ...
                if self.ticket_flag:
                    msg = '{} さん \nあと {}秒'.format(self.ticket, int(LIMIT_MIN*60 - t_delta.seconds))
                else:
                    msg = '{} さん \n操作チケット {}秒'.format(self.ticket, int(VALIDATE_SEC - t_delta.seconds))
        else:
            # フリータイム：今まで通り
            self.ticket = None
            self.selected_time = datetime.now()
